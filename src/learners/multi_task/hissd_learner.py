    ...
    def train_planner(self, batch: EpisodeBatch, t_env: int, episode_num: int, task: str, 
        ...
        for t in range(batch.max_seq_length - self.c):
            out_h, obs_loss = self.mac.forward_planner(
                batch, t=t, task=task, actions=actions[:, t], training=True, loss_out=True)
            value_out_h = self.mac.forward_planner_feedforward(out_h, forward_type='value')
        ...
        for i in range(self.c):
            out_h, _ = self.mac.forward_planner(batch, t=t+i, task=task, actions=actions[:, t+i])
            value_out_h = self.mac.forward_planner_feedforward(out_h, forward_type='value')
            mac_value.append(value_out_h)
        # mac_value = th.stack(mac_value, dim=1)
        # with th.no_grad():
        ...
        for t in range(batch.max_seq_length):
            ...
            with th.no_grad():
                target_value = self.target_mac.forward_value_skill(batch, mac_value[t], task=task)
            value_pre.append(value)
            target_value_pre.append(target_value)
        ...
        td_error = discount * target_mixed_values[:, self.c:].detach() + cs_rewards[:, :-self.c] - \
            mixed_values[:, :-self.c]
        ...
        weight = th.clamp_max(weight, 100.).detach()
