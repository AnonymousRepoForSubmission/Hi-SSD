...
class HISSDSMAC:
    ...
    def forward_global_action(self, ep_batch, obs_emb, disrc_h, t, task, test_model=False):
        bs = ep_batch.batch_size
        agent_inputs = self._build_inputs(ep_batch, t, task)
        disrc_h = disrc_h.reshape(-1, 1, self.main_args.entity_embed_dim)
        act_out, self.hidden_states_dec, self.hidden_states_plan, cls_out = self.agent.forward_action(
            agent_inputs, obs_emb, disrc_h, self.hidden_states_dec, self.hidden_states_plan, task, t=t)
        return act_out.reshape(bs, 1, self.task2n_agents[task], -1), \
        cls_out.reshape(bs, 1, self.task2n_agents[task], self.cls_dim)
    ...
    def forward_seq_action(self, ep_batch, t, task, mask=False, test_model=False):
        ...
        agent_seq_outs, self.hidden_states_dec, self.hidden_states_plan = self.agent.forward_seq_action(
            agent_seq_inputs, self.hidden_states_dec, self.hidden_states_plan, task, mask, t, actions)
    ...
    def forward_planner(self, ep_batch, t, task, 
                        actions=None, test_mode=False, training=False, hrl=False, loss_out=False):
        if t % self.c_step == 0 or hrl == False:
            ...
            out_h, self.hidden_states_plan, obs_loss = self.agent.forward_planner(
                agent_inputs, self.hidden_states_plan, t, task, actions=actions, next_inputs=next_inputs, loss_out=loss_out)
    ...
    def forward_discriminator(self, ep_batch, t, task, test_mode=False):
        ...
        return ssl_out.reshape(ep_batch.batch_size, self.task2n_agents[task], -1), \
    ...
    def forward(self, ep_batch, t, task, test_mode=False):
        ...
        if t % self.c_step == 0:
            agent_outs, self.hidden_states_plan, self.hidden_states_dec, self.hidden_states_dis, self.skill = self.agent(
                agent_inputs, self.hidden_states_plan, self.hidden_states_dec, self.hidden_states_dis, 
                t, task, None, actions=actions, local_obs=None)
            # local_obs=local_agent_inputs)
        else:
            agent_outs, self.hidden_states_plan, self.hidden_states_dec, self.hidden_states_dis, _ = self.agent(
                agent_inputs, self.hidden_states_plan, self.hidden_states_dec, self.hidden_states_dis, 
                t, task, self.skill, actions=actions, local_obs=None)
            # local_obs=local_agent_inputs)
    ...
    def init_hidden(self, batch_size, task):
        ...
        hidden_states_value, hidden_states_dec, hidden_states_plan, hidden_states_dis = self.agent.init_hidden()
        self.hidden_states_value = hidden_states_value.unsqueeze(0).expand(batch_size, n_agents, -1)
        self.hidden_states_dec = hidden_states_dec.unsqueeze(0).expand(batch_size, n_agents, -1)
        self.hidden_states_plan = hidden_states_plan.unsqueeze(0).expand(batch_size, n_agents, -1)
        self.hidden_states_dis = hidden_states_dis.unsqueeze(0).expand(batch_size, n_agents, -1)
